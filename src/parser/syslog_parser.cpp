...
int parse_pri(std::string_view& sv) {
    ...
    if (digits == 0 || sv.empty() || sv[0] != '>') return -1;
    if (pri > 191) return -1;
    sv.remove_prefix(1);
    return pri;
}
...
bool is_month_prefix(std::string_view sv) {
    ...
    for (const char* m : kMonths)
        if (sv[0] == m[0] && sv[1] == m[1] && sv[2] == m[2]) return true;
    return false;
}
...
std::optional<std::string> parse_3164_timestamp(std::string_view& sv) {
    if (!is_month_prefix(sv)) return std::nullopt;
    std::size_t p = 3;
    if (p >= sv.size() || sv[p] != ' ') return std::nullopt;
    while (p < sv.size() && sv[p] == ' ') ++p;
    const std::size_t day0 = p;
    while (p < sv.size() && std::isdigit((unsigned char)sv[p])) ++p;
    if (p - day0 == 0 || p - day0 > 2) return std::nullopt;
    if (p >= sv.size() || sv[p] != ' ') return std::nullopt;
    while (p < sv.size() && sv[p] == ' ') ++p;
    if (p + 8 > sv.size()) return std::nullopt;
    const auto t = sv.substr(p, 8);
    if (!std::isdigit((unsigned char)t[0]) || !std::isdigit((unsigned char)t[1]) ||
        t[2] != ':' ||
        !std::isdigit((unsigned char)t[3]) || !std::isdigit((unsigned char)t[4]) ||
        t[5] != ':' ||
...
parse_3164_tag(std::string_view& sv) {
    ...
    if (bracket != std::string_view::npos && !tag_part.empty() &&
        tag_part.back() == ']') {
        auto app = std::string(tag_part.substr(0, bracket));
        auto pid = std::string(tag_part.substr(bracket + 1,
                                               tag_part.size() - bracket - 2));
        return { app.empty() ? std::nullopt : std::make_optional(std::move(app)),
                 pid.empty() ? std::nullopt : std::make_optional(std::move(pid)) };
    }
    while (!tag_part.empty() && tag_part.back() == ' ') tag_part.remove_suffix(1);
    return { tag_part.empty() ? std::nullopt
                              : std::make_optional(std::string(tag_part)),
             std::nullopt };
}
...
bool try_rfc5424(std::string_view data, SyslogMessage& msg) {
    ...
    std::size_t vi = 0;
    while (vi < sv.size() && std::isdigit((unsigned char)sv[vi])) ++vi;
    if (vi == 0 || vi >= sv.size() || sv[vi] != ' ') return false;
    int ver = 0;
    for (std::size_t j = 0; j < vi; ++j) ver = ver * 10 + (sv[j] - '0');
    if (ver != 1) return false;
    sv.remove_prefix(vi + 1);

    // Need at least 4 spaces for the 5 required header fields
    if (std::count(sv.begin(), sv.end(), ' ') < 4) return false;
...
bool try_rfc3164(std::string_view data, SyslogMessage& msg) {
    ...
    std::optional<std::string> hostname;
    {
        const auto sp = sv.find(' ');
        if (sp == std::string_view::npos) {
            if (!sv.empty()) hostname = std::string(sv);
            sv = {};
        } else {
...
SyslogMessage parse_syslog(std::string_view data) {
    ...
    SyslogMessage msg;
    if (try_rfc5424(data, msg)) return msg;
    if (try_rfc3164(data, msg)) return msg;

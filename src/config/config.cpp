...
const std::unordered_map<std::string, int> kFacilityNames = {
    ...
    {"ftp",      11},
    {"ntp",      12},
    {"audit",    13}, {"logaudit", 13},
    {"alert",    14}, {"logalert", 14},
    {"clock2",   15},
    {"local0",   16},
    {"local1",   17},
    {"local2",   18},
...
std::vector<int> parse_facilities(const std::string& raw) {
    ...
    while (std::getline(ss, token, ',')) {
        ...
        if (token == "*") return {}; // wildcard → empty = all
...
uint64_t parse_size(const std::string& raw) {
    ...
    std::size_t i = 0;
    while (i < raw.size() && std::isdigit((unsigned char)raw[i])) ++i;
    if (i == 0)
    ...
    uint64_t mult = 1;
    if      (unit == "b" || unit.empty()) mult = 1ULL;
...
OutputConfig parse_output(const std::string& name,
    ...
    if (out.text_file.empty() && out.jsonl_file.empty())
        throw std::runtime_error(
            "[output." + name + "] must specify text_file, jsonl_file, or both");
    ...
    if (out.max_files < 0)
        throw std::runtime_error(
            "[output." + name + "] max_files must be >= 0");
...
Config load_config(const std::string& path) {
    ...
    {
        const int port = tree.get<int>("server.udp_port",
                                      static_cast<int>(cfg.udp_port));
    ...
    // Iterate the top-level tree and pick every key with the "output." prefix.
    {
        ...
        for (auto& [key, sec] : tree) {
            ...
            if (!seen.insert(name).second)
                throw std::runtime_error(
                    "Duplicate output section name: '" + name + "'");
    ...
    if (auto fwd_node = tree.get_child_optional("forwarding")) {
        ...
        cfg.forwarding.max_message_size = f.get<uint32_t>(
            "max_message_size", cfg.forwarding.max_message_size);
        cfg.forwarding.facilities       = parse_facilities(
            f.get<std::string>("facility", "*"));

        const int port = f.get<int>("port",
                                   static_cast<int>(cfg.forwarding.port));
        if (port <= 0 || port > 65535)
            throw std::runtime_error(
                "Invalid forwarding port: " + std::to_string(port));

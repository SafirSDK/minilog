#!/usr/bin/env python3
"""Verify that every .cpp and .hpp file under src/server/, src/send/, tests/server/
and tests/send/ contains the expected copyright header.  Exits non-zero if any file
is missing it."""
...
for pattern in ("**/*.cpp", "**/*.hpp"):
    for directory in ("src/server", "src/send", "tests/server", "tests/send"):

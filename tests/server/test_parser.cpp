...
BOOST_AUTO_TEST_CASE(empty_app_name_in_bracket_tag)
{
    // Tag "[123]:" — empty app name before the bracket, PID inside.
    const auto m = parse("<13>Jan 12 15:04:05 host [123]: message body");
    BOOST_TEST((m.protocol == Protocol::RFC3164));
    BOOST_TEST(!m.appName.has_value());
    BOOST_TEST(*m.procId == "123");
    BOOST_TEST(m.message == "message body");
...
BOOST_AUTO_TEST_CASE(empty_pid_in_bracket_tag)
{
    // Tag "app[]:" — empty PID inside brackets.
    const auto m = parse("<13>Jan 12 15:04:05 host app[]: message body");
    BOOST_TEST((m.protocol == Protocol::RFC3164));
    BOOST_TEST(*m.appName == "app");
    BOOST_TEST(!m.procId.has_value());
    BOOST_TEST(m.message == "message body");
